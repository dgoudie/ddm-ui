...
export function MixedDrinkListItem({
  ...
  const deleteRecord = React.useCallback(() => {
    const fn = async () => {
      try {
        await deleteMixedDrink(mixedDrink._id as unknown as string);
    ...
    <React.Fragment>
      ...
      >
        ...
        >
          ...
          <Text color='text.secondary' fontSize={2} fontStyle='italic'>
            ${mixedDrink.price.toFixed(2)}
          </Text>
        </Grid>
        <div>
          {mixedDrink.requiredBeersOrLiquors.map((ingredient, index) => (
            <React.Fragment key={ingredient._id as unknown as string}>
              <Text
                fontSize={2}
                color={ingredient.inStock ? 'text.primary' : 'text.danger'}
              >
                {ingredient.name}
                {index < mixedDrink.requiredBeersOrLiquors.length - 1 && ', '}
              </Text>
            </React.Fragment>
          ))}
        </div>
        ...
        <BorderBox bg='bg.tertiary' p={3} mb={3}>
          ...
          {loggedIn && (
            ...
            >
              ...
              <ButtonDanger onClick={() => setConfirmDelete(true)}>
